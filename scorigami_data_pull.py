...
for year in range(1869,2022):
    ...
    for i in api_response:
        if i.home_team in FBS_schools or i.away_team in FBS_schools:
            ...
            if score_comb not in scores and i.home_points is not None:
                scores.append(score_comb)
                home_score.append(int(i.home_points))
                away_score.append(int(i.away_points))
...
# Export data frame
df.to_csv('historical_scorigami_scores.csv', index=False)
